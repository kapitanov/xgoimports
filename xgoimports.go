...
// https://cs.opensource.google/go/x/tools/+/refs/tags/v0.34.0:cmd/goimports/goimports.go
//
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: xgoimports [flags] [path ...]\n")
	flag.PrintDefaults()
	fmt.Fprintf(os.Stderr, "\nxgoimports is a fork of the original goimports tool built by Go Authors\n")
	fmt.Fprintf(os.Stderr, "\noriginal source is available at: %s\n", "https://cs.opensource.google/go/x/tools/+/refs/tags/v0.34.0:cmd/goimports")
	os.Exit(2)
}
...
func processFile(filename string, in io.Reader, out io.Writer, argType argumentType) error {
	...
	src = preprocessFile(filename, src, opt)
